...
function jtheme_form_fields($fields = '', $args = '') {	
	$defaults = array(
	...
	foreach ($fields as $field) {
		...
		elseif($type == 'fields' ) {
			...
			jtheme_form_fields( $field['fields'], wp_parse_args( $args, $defaults ) );
			echo '</td></tr>';
		} 
		elseif(!empty($type)) {
...
function jtheme_form_widget($fields = array(), $field_args_callback = '') {
...
function jtheme_form_field($args = '') {
	if(empty($args['type']))
		return;
	...
	if($args['type'] == 'upload') {
		$class .= ' jtheme-upload-text';
	} elseif( $args['type'] == 'color' ) {
		$class .= 'jtheme-color-input';
	}
	...
	elseif($type == 'upload') {
		...
		$output = "<input{$type}{$name}{$value}{$id}{$class}{$style} />";
		$output .= ' &nbsp; <a title="" class="button jtheme-upload-button" href="'.get_upload_iframe_src('image').'">Upload</a> <a href="#" class="button jtheme-remove-button">Remove</a> <div class="jtheme-upload-preview"></div>';
	} 
	...
	elseif($type == 'color') {
		...
		$output = "<span class='jtheme-color-handle colorSelector'>&nbsp;</span> <input{$type}{$name}{$value}{$id}{$class}{$style}>";
	}
	...
	elseif($type == 'checkboxes' && is_array($options)) {
		...
		$output = '<div class="jtheme-checkboxes">' . implode($args['sep'] ? $args['sep'] : '<br />', $output) . '</div>';
	}
...
function jtheme_field_options( $fields = array() ) {
	$options = array();
	...
	foreach($fields as $field) {
		...
		if( !empty($field['fields']) && $field['type'] == 'fields' ) {
			$options = array_merge_recursive( $options, jtheme_field_options($field['fields']) );
		} else {
			if(empty($field['name']) )
				continue;
...
function jtheme_instance_fields( $fields, $instance_type = '', $object = '') {
	$new_fields = array();
	
	foreach($fields as $field)
		$new_fields[] = jtheme_instance_field($field, $instance_type, $object);
		
	return $new_fields;
}

function jtheme_instance_field( $field, $instance_type = '', $object = '') {
	global $post;
		
		if(!empty($field['fields']) && $field['type'] == 'fields') {
			$field['fields'] = jtheme_instance_fields($field['fields'], $instance_type);
...
function jtheme_get_logic_fields( $args = '' ) {
	...
	$fields['global'] = array(
		'name' => 'global',
		'title' => __('Global Settings', 'jtheme')
	);
	
	/* Front Page **/
	$fields['front_page'] = array(
		'name' => 'front_page',
		'title' => __('Front Page', 'jtheme')
	);
	
	// Home
	$fields['home'] = array(
		'name' => 'home',
		'title' => __('Blog Home', 'jtheme')
	);
	...
	foreach($post_types as $type => $obj) {
		$fields['single_' . $type] = array(
			'name' => 'single_' . $type,
			'title' => sprintf( __('Single %s', 'jtheme'), $obj->labels->singular_name ),
			'group' => 'single',
		);
	}
	...
	foreach($post_types as $type => $obj) {
		$fields['archive_' . $type] = array(
			'name' => 'archive_' . $type,
			'title' => sprintf( __('Post Type Archive: %s', 'jtheme' ), $obj->labels->singular_name ),
	...
	foreach($taxonomies as $tax => $obj) {
		$fields['tax_' . $tax] = array(
			'name' => 'tax_' . $tax,
			'title' => sprintf( __('Taxonomy Archive: %s', 'jtheme'), $obj->labels->singular_name ),
			'group' => 'tax'
		);
	}
	...
	$fields['author'] = array(
		'name' => 'author',
		'title' => __('Author Archive', 'jtheme')
	);
	
	/* Date Archives */
	$fields['date'] = array(
		'name' => 'date',
		'title' => __('Date Archive', 'jtheme')
	);
	
	/* Search Pages */ 
	$fields['search'] = array(
		'name' => 'search',
		'title' => __('Search Result Page', 'jtheme')
	);

	/* 404 */
	$fields['404'] = array(
		'name' => '404',
		'title' => __('404 Page', 'jtheme')
	);
...
function jtheme_get_logic_option( $settings = '' ) {
	global $wp_query;
	
	if(empty($settings) || !is_array($settings))
		return;
	
	$defaults = array_keys( jtheme_get_logic_fields() );
