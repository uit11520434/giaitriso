...
class jtheme_Widget_Tweets extends WP_Widget {

    function __construct() {
		$widget_ops = array('classname' => 'widget-tweets', 'description' => __( 'Display your latest tweets.', 'jtheme') );
        $control_ops = array('width' => 400);
        parent::WP_Widget("jtheme-tweets-widget", __('JTHEME-Latest-Tweets', 'jtheme'), $widget_ops, $control_ops);	
		
		$this->options = array(
			array(
				'label' => __( 'Twitter Authentication Options', 'jtheme' ),
				'type'	=> 'separator', 	'notes' => __('Get them creating your Twitter Application', 'jtheme' ).' <a href="https://dev.twitter.com/apps" target="_blank">'.__('here', 'jtheme' ).'</a><br /><br />'	),
			array(
			...
			array(
				...
				'type'	=> 'text',	'default' => ''			),
			array(
				'label' => __( 'Twitter Data Options', 'jtheme' ),
				'type'	=> 'separator'			),
			array(
				'name'	=> 'username',		'label'	=> __( 'Twitter Username', 'jtheme' ),
				'type'	=> 'text',	'default' => ''			),
			array(
				'name'	=> 'count',			'label'	=> __( 'Number of Tweets', 'jtheme' ),
				'type'	=> 'text',	'default' => 5			),
			array(
				'name'	=> 'exclude_replies',		'label'	=> __( 'Exclude replies?', 'jtheme' ),
				'type'	=> 'checkbox',	'default' => true	),
			array(
				'name'	=> 'include_rts',		'label'	=> __( 'Include retweets?', 'jtheme' ),
				'type'	=> 'checkbox',	'default' => false	),
			array(
				'name'	=> 'exclude_keywords',		'label'	=> __( 'Exclude tweets containing these keywords (Separate keywords with commas)', 'jtheme' ),
				'type'	=> 'text',	'default' => ''			),
			array(
				'label' => __( 'Widget Title Options', 'jtheme' ),
				'type'	=> 'separator'			),
			array(
				'name'	=> 'title',	'label'	=> __( 'Title', 'jtheme' ),
				'type'	=> 'text',	'default' => __( 'Last Tweets', 'jtheme' )			),
			array(
				'name'	=> 'link_title',	'label'	=> __( 'Link above Title with Twitter user', 'jtheme' ),
				'type'	=> 'checkbox',	'default' => false			),
			array(
				'label' => __( 'Display Options', 'jtheme' ),
				'type'	=> 'separator'			),
			array(
				'name'	=> 'linked',		'label'	=> __( 'Show this linked text at the end of each Tweet', 'jtheme' ),
				'type'	=> 'text',	'default' => ''			),
			array(
				'name'	=> 'show_time',	'label'	=> __( 'Show timestamps?', 'jtheme' ),
				'type'	=> 'checkbox',	'default' => true			),
			array(
				'name'	=> 'show_source',	'label'	=> __( 'Show source?', 'jtheme' ),
				'type'	=> 'checkbox',	'default' => false			),
			array(
				'name'	=> 'text_before_source',		'label'	=> __( 'Text before source', 'jtheme' ),
				'type'	=> 'text',	'default' => 'from'			),
			array(
				'name'	=> 'show_avatar',	'label'	=> __( 'Show avatar?', 'jtheme' ),
				'type'	=> 'checkbox',	'default' => false			),			
			array(
				'name'	=> 'show_otweet_avatar', 'label'	=> __( 'Show original tweet avatar for retweets?', 'jtheme' ),
				'type'	=> 'checkbox',	'default' => false			),	
			array(
				'label' => __( 'Links options', 'jtheme' ),
				'type'	=> 'separator'			),
			array(
				'name'	=> 'autolink_username',	'label'	=> __( 'Autolink @username?', 'jtheme' ),
				'type'	=> 'checkbox',	'default' => true			),
			array(
				'name'	=> 'autolink_hashtag',	'label'	=> __( 'Autolink #hashtag?', 'jtheme' ),
				'type'	=> 'checkbox',	'default' => true			),
			array(
				'name'	=> 'autolink_mail',	'label'	=> __( 'Autolink mail address?', 'jtheme' ),
				'type'	=> 'checkbox',	'default' => true			),
			array(
				'name'	=> 'autolink_url',	'label'	=> __( 'Autolink URLs?', 'jtheme' ),
				'type'	=> 'checkbox',	'default' => true			),
			array(
				'name'	=> 'replace_url_text',	'label'	=> __( 'Replace url text inside tweets with fixed text (e.g. "-->")', 'jtheme' ),
				'type'	=> 'text',	'default' => ''			),
			array(
				'name'	=> 'link_target_blank',	'label'	=> __( 'Open links in new window?', 'jtheme' ),
				'type'	=> 'checkbox',	'default' => false			),
			array(
				'label' => __( 'Widget footer options', 'jtheme' ),
				'type'	=> 'separator'			),
			array(
				'name'	=> 'link_user',		'label'	=> __( 'Show a footer link to the Twitter user profile', 'jtheme' ),
				'type'	=> 'checkbox',	'default' => false			),
			array(
				'name'	=> 'link_user_text',	'label'	=> __( 'Text for footer link', 'jtheme' ),
				'type'	=> 'text',	'default' => 'Follow me on Twitter'			),
			array(
				'label' => __( 'Debug options', 'jtheme' ),
				'type'	=> 'separator'			),
			array(
				'name'	=> 'encode_utf8',	'label'	=> __( 'Force UTF8 Encode (use it only if having issues)', 'jtheme' ),
				'type'	=> 'checkbox',	'default' => false			),
		);
    }
	...
	function form($instance) {
		...
		echo '<div class="jtheme-widget-tweets-form">';
	...
	protected function get_tweets($options) {
		...
		if(empty($options['username']))
			return new WP_Error('no_username', __('Twitter username is not configured', 'jtheme'));

		if((int)$options['count']<=0)
			return new WP_Error('valid_count', __('Number of tweets is not valid', 'jtheme'));

		if(empty($options['consumer_key']) || empty($options['consumer_secret']) || empty($options['access_token']) || empty($options['access_token_secret']))
			return new WP_Error('no_auth', __('Twitter Authentication data is incomplete', 'jtheme'));
		...
		if(empty($tweets)) {
			try{
				...
				return new WP_Error('fail', __('Error retrieving tweets', 'jtheme'));
			}
		} 
		...
		if(!isset($tweets['errors']) && (count($tweets) >= 1) && $tweets['httpstatus'] == '200') {
			...
			return new WP_Error('data_error', __('Twitter data error:','jtheme').' '.$tweets['errors'][0]['message'].'<br />');
			...
			return new WP_Error('no_respond', __('Error: Twitter did not respond. Please wait a few minutes and refresh this page.', 'jtheme'));
		}
	...
	protected function display_tweets($options) {
		...
		foreach($tweets as $tweet) {
			...
			if($options['show_time']) {
				...
				$human_time = (abs(time()-$time)) < 86400 ? sprintf( __('%s ago', 'jtheme'), human_time_diff( $time )) : date(__('M d', 'jtheme'), $time);
				
				$meta['time'] = '<span class="tweet-timestamp" title="' . date(__('Y/m/d H:i', 'jtheme'), $time) . '">' . $human_time . '</span>';
			}
			...
			$out .= '<div class="'.$item_class.' cbp-qtcontent">'.$avatar.'<span class="tweet-content">'.$text.$meta.'</span>'.'</div>';
			
			$i++;
		}
		
		$out = '<div id="cbp-qtrotator" class="cbp-qtrotator">'.$out.'</div>';
		
		if ($options['link_user'])
			$out .= '<div class="jtheme-link-user"><a href="http://twitter.com/' . $options['username'] . '" '.$link_target.'>'.$options['link_user_text'].'</a></div>';
...
// Register widget
add_action('widgets_init', create_function('', 'return register_widget("jtheme_Widget_Tweets");'));
