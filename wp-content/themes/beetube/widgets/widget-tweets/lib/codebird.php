...
class Codebird
{
    ...
    /**
     * The API enjthemeoint to use
     */
    private $_enjthemeoint = 'https://api.twitter.com/1.1/';

    /**
     * The API enjthemeoint to use for OAuth requests
     */
    private $_enjthemeoint_oauth = 'https://api.twitter.com/';

    /**
     * The API enjthemeoint to use for untransitioned methods
     */
    private $_enjthemeoint_old = 'https://api.twitter.com/1/';
    ...
    public function oauth_authenticate()
    {
        ...
        return $this->_enjthemeoint_oauth . 'oauth/authenticate?oauth_token=' . $this->_url($this->_oauth_token);
    }
    ...
    public function oauth_authorize()
    {
        ...
        return $this->_enjthemeoint_oauth . 'oauth/authorize?oauth_token=' . $this->_url($this->_oauth_token);
    }
    ...
    {
        // multi-HTTP method enjthemeoints
        switch($method) {
    ...
    /**
     * Detects if API call should use the old enjthemeoint
     *
     * @param string $method The API method to call
     *
    ...
    /**
     * Builds the complete API enjthemeoint url
     *
     * @param string $method           The API method to call
     ...
     */
    private function _getEnjthemeoint($method, $method_template)
    {
        if (substr($method, 0, 6) == 'oauth/') {
            $url = $this->_enjthemeoint_oauth . $method;
        } elseif ($this->_detectOld($method_template)) {
            $url = $this->_enjthemeoint_old . $method . '.json';
        } else {
            $url = $this->_enjthemeoint . $method . '.json';
        }
        return $url;
    }
    ...
    private function _callApi($httpmethod, $method, $method_template, $params = array(), $multipart = false)
    {
        ...
        $url = $this->_getEnjthemeoint($method, $method_template);
        $ch  = false;
        if ($httpmethod == 'GET') {
