...
class jtheme_Widget_User_Bio extends WP_Widget {

	function __construct() {
		$widget_ops = array('classname' => 'widget-user-bio', 'description' => __('Display the "Biographical Info" of a user.', 'jtheme'));
		$control_ops = array('width' => 400, 'height' => 350);
		
		parent::__construct('jtheme-user-bio', __('JTHEME-User-Detail', 'jtheme'), $widget_ops, $control_ops);
	}
	...
	function form( $instance ) {
		...
		<p class='description'><?php _e("'%s' in string will be replaced with user's name.", 'jtheme'); ?></p>
		
		<p><label for="<?php echo $this->get_field_id('user_id'); ?>"><?php _e('User ID:', 'jtheme'); ?></label>
		...
		<p class='description'><?php _e("Enter a length number if you want to limit bio length, e.g. 300.", 'jtheme'); ?></p>

		<p><input id="<?php echo $this->get_field_id('strip_tags'); ?>" name="<?php echo $this->get_field_name('strip_tags'); ?>" type="checkbox" <?php checked($strip_tags); ?> />&nbsp;<label for="<?php echo $this->get_field_id('strip_tags'); ?>"><?php _e('Strip HTML Tags?', 'jtheme'); ?></label></p>
	<?php }
}

// Register Widget
add_action('widgets_init', 'register_jtheme_widget_user_bio');
function register_jtheme_widget_user_bio() {
	register_widget('jtheme_Widget_User_Bio');
}